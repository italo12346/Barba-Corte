...
const ColaboradorViewModal = ({ open, onClose, colaborador, loading = false }) => {
  ...
  useEffect(() => {
    if (colaborador?.especialidades?.length) {
      dispatch(loadServicosColaborador(colaborador.especialidades));
    }
  }, [colaborador, dispatch]);
  
  if (!colaborador) return null;
  // =========================================
  // MAPA DE SERVIÇOS
  // =========================================
  const servicosMap = servicosColaborador ?? {};
        ...
        {!loading && !loadingServicosColaborador && (
          <div className="container-fluid">
            ...
            <div className="card shadow-sm mb-4">
              <div className="card-header bg-dark text-white">Dados do Colaborador</div>
              <div className="card-body row g-3">
                <Info label="Telefone" value={formatarTelefone(colaborador.telefone)} />
                <Info label="Sexo" value={colaborador.sexo} />
                <Info label="Nascimento" value={formatarData(colaborador.dataNascimento)} />
                <Info label="Cadastro" value={colaborador.dataCadastro} />
                <Info
                  label="Status"
                  value={
                    <span className={`badge ${getStatusBadge(colaborador.status)}`}>
            ...
            <div className="card shadow-sm mb-4">
              ...
              <div className="card-body row g-3">
                <Info
                  ...
                  value={colaborador.mercadoPago?.connected ? "Conectado" : "Não conectado"}
                />
              </div>
            </div>
            ...
            <div className="card shadow-sm">
              <div className="card-header bg-dark text-white">Especialidades</div>
              <div className="card-body">
                {colaborador.especialidades?.length === 0 && (
                  <div className="text-muted">Nenhuma especialidade cadastrada</div>
                )}
