...
export default function Perfil() {
  ...
  const handleSalvar = async () => {
    ...
    setSalvando(true);
    try {
      await api.put(`/cliente/${cliente!._id}`, {
        nome:     form.nome,
      ...
      >
        ...
        >
          ...
          <View style={styles.hero}>
            ...
            {/* Avatar */}
            <TouchableOpacity style={styles.avatarWrapper} onPress={handleTrocarFoto} activeOpacity={0.85}>
              {uploadingFoto ? (
          ...
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              ...
              <View style={styles.badge}>
                <Text style={styles.badgeText}>{AGENDAMENTOS_MOCK.length}</Text>
              </View>
            </View>

            {AGENDAMENTOS_MOCK.map((ag) => {
              const cfg = STATUS_CONFIG[ag.status];
              return (
                <View key={ag.id} style={styles.agCard}>
                  <View style={styles.agLeft}>
                    <View style={styles.agIconBox}>
                      <MaterialIcons name="content-cut" size={18} color="#6b21a8" />
                    </View>
                  </View>
                  <View style={styles.agInfo}>
                    <Text style={styles.agSalao}>{ag.salao}</Text>
                    <Text style={styles.agServico}>{ag.servico}</Text>
                    <View style={styles.agDateRow}>
                      <Ionicons name="calendar-outline" size={12} color="#999" />
                      <Text style={styles.agDate}>{ag.data} às {ag.hora}</Text>
                    </View>
                  </View>
                  <View style={[styles.agStatus, { backgroundColor: cfg.bg }]}>
            ...
            <View style={styles.mockAviso}>
              <Ionicons name="information-circle-outline" size={14} color="#9E9E9E" />
              <Text style={styles.mockAvisoText}>
                Histórico real disponível em breve
              </Text>
            </View>
          </View>
...
function Campo({ label, icon, value, editando, keyboardType = 'default', onChangeText }: CampoProps) {
  return (
    <View style={styles.campoWrapper}>
      <View style={styles.campoLabelRow}>
...
const styles = StyleSheet.create({
  ...
  // Hero
  hero: { alignItems: 'center', paddingTop: 40, paddingBottom: 36, backgroundColor: '#1a0a2e' },
  heroBg: {
  ...
  avatarFallback: {
    ...
    borderWidth: 3, borderColor: '#9333ea',
  },
  avatarLoading: {
    width: 100, height: 100, borderRadius: 50,
  ...
  cameraBtn: {
    ...
    justifyContent: 'center', alignItems: 'center',
    borderWidth: 2, borderColor: '#1a0a2e',
  },
  heroNome:  { fontSize: 22, fontWeight: '800', color: '#FFF', letterSpacing: 0.3 },
  ...
  cancelarText: { color: '#9E9E9E', fontSize: 13, textDecorationLine: 'underline' },
  ...
  campoLabelRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 },
  campoLabel:    { fontSize: 11, fontWeight: '700', color: '#6b21a8', textTransform: 'uppercase', letterSpacing: 1 },
  campoValor:    { fontSize: 15, color: '#1a0a2e', fontWeight: '600', paddingLeft: 2 },
  campoInput: {
    fontSize: 15, color: '#1a0a2e', fontWeight: '600',
  ...
  agDateRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 4 },
